...
public class Matrix3f {
  ...
  public static Vec3 multVec(Matrix3f m, Vec3 vec) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      float[] column = { (float)vec.field_72450_a, (float)vec.field_72448_b, (float)vec.field_72449_c };
      for (int sm = 0; sm < 3; ) {
        retVec[i] = retVec[i] + row[sm] * column[sm];
        sm++;
      } 
    } 
    return Vec3.func_72443_a(retVec[0], retVec[1], retVec[2]);
  }
  ...
  public static Matrix3f multMatrix(Matrix3f m1, Matrix3f m2) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        for (int sm = 0; sm < 3; ) {
          retMat.matrix[i][j] = retMat.matrix[i][j] + row[sm] * column[sm];
          sm++;
        } 
      } 
    } 
