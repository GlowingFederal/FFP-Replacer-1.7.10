...
public class AnimTankTrack {
  ...
  public float getTrackLength() {
    ...
    for (int i = 0; i < this.points.size(); i++)
      length += distBetweenPoints(this.points.get(i), this.points.get((i == 0) ? (this.points.size() - 1) : (i - 1))); 
    return length;
  }
  ...
  public int getTrackPart(float distance) {
    ...
    for (int i = 0; i < this.points.size(); i++) {
      if (length >= distance)
        return i; 
      length += distBetweenPoints(this.points.get(i), this.points.get((i == this.points.size() - 1) ? 0 : (i + 1)));
    } 
    return 0;
  }
  ...
  public float getProgressAlongTrackPart(float distance, int trackPart) {
    ...
    for (int i = 0; i < trackPart + 1; i++) {
      if (i != 0)
        length += distBetweenPoints(this.points.get(i - 1), this.points.get(i)); 
    } 
    return length;
  }
  ...
  public Vector3f getPositionOnTrack(float distance) {
    ...
    Vector3f p2 = this.points.get((trackPart == 0) ? (this.points.size() - 1) : (trackPart - 1));
    Vector3f p1 = this.points.get(trackPart);
    float partLength = distBetweenPoints(p2, p1);
    float prog = distance - getProgressAlongTrackPart(distance, (trackPart == 0) ? (this.points.size() - 1) : (trackPart - 1));
