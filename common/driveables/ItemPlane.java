...
public class ItemPlane extends Item implements IPaintableItem {
    ...
    try {
      ...
      if (file1 != null && file1.exists()) {
        ...
        for (EnumDriveablePart part : EnumDriveablePart.values()) {
          ...
          tags.func_74768_a(part.getShortName() + "Crew", (this.type.crew.get(part) == null) ? 0 : ((CollisionBox)this.type.crew.get(part)).crew);
    ...
    if (movingobjectposition.field_72313_a == MovingObjectPosition.MovingObjectType.BLOCK) {
      ...
      if (this.type.placeableOnLand || block instanceof net.minecraft.block.BlockLiquid) {
        if (!world.field_72995_K) {
          ...
          if (data != null)
            world.func_72838_d(new EntityPlane(world, i + 0.5D, j + 2.5D, k + 0.5D, entityplayer, this.type, data)); 
        } 
  ...
  public Entity spawnPlaneAngled(World world, double x, double y, double z, float yaw, float pitch, float roll, ItemStack stack) {
    DriveableData data = getPlaneData(stack, world);
    if (data != null) {
      Entity entity = new EntityPlane(world, x, y, z, yaw, pitch, roll, this.type, data);
      if (!world.field_72995_K)
        world.func_72838_d(entity); 
      return entity;
    } 
